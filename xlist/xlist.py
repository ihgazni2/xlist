#docs
#format https://sphinxcontrib-napoleon.readthedocs.io/en/latest/
...
###
# 提供统一的_inplace_wrapper, keep_ptr_wrapper
# 对于能改变顺序的操作,比如sort,提供统一的recordize,unrecordize
# 对用于map 的f 函数,提供统一的recordize_func_wrapper
# 
###
...
def _keep_ptr_wrapper(func):
    ...
    exzamples:
        ...
        @_keep_ptr_wrapper
        def do_plus_one(ol):
            nl = list(map(lambda ele:ele+1,ol))
            return(nl)
        ...
        nl0
        id(nl0)
        nl1 = do_plus_one(ol,keep_ptr=True)
        nl1
        id(nl1)
    ...
    def wrapper(*args,**kwargs):
        if('keep_ptr' in kwargs):
            keep_ptr = kwargs['keep_ptr']
        else:
            keep_ptr = False
        args = list(args)
        ol = args[0]
...
def init(lngth,value=None):
    '''init a list with a value
    ...
    lngth : int
        list length
    value : object
        fill the list with the value,the value is deepcopied
        default = None
    ...
    list
    '''
    rslt = [copy.deepcopy(value)  for i in range(lngth)]
    return(rslt)

def init_with(lngth,value=None):
    '''init a list with a value
    ...
    lngth : int
        list length
    value : object
        fill the list with the value,the value is NOT deepcopied
        default = None
    ...
    list
    '''
    rslt = [value for i in range(lngth)]
    return(rslt)



def init_range(start,end,step=1):
    '''init a list from start to end with a step interval
...
def range_left(*args):
    ...
    This function is from https://lodash.com/docs/4.17.11#range
    ...
    exzamples:
        ...
        Traceback (most recent call last):
          ...
          File "<stdin>", line 22, in range_left
    ...
    step  : int
        the value to increment or decrement by,default = 1
...
def range_right(*args):
    '''generate a range array in descending order

    This function is from https://lodash.com/docs/4.17.11#rangeRight
    This method is like range_left except that it populates values in descending order.
    ...
    '''
    rslt = range_left(*args)
    rslt.reverse()
    return(rslt)


##CONVERT
@eftl.deepcopy_and_keep_ptr_wrapper

def cast_array(obj):
    '''Casts value as an array if it's not one.

    This function is from https://lodash.com/docs/4.17.11#castArray
...
def is_array(ol):
    ...
    bool
    '''
    cond = isinstance(ol,list)
    return(cond)


def uniform_index(index,length):
    '''convert a number to a >= index based on length
...
# https://lodash.com/docs/4.17.11#fill
...
def divide(ol,interval):
    ...
    '''
    length = ol.__len__()
    seqs = init_range(0,length,interval)
